...
void Hahuyeon::render(HDC hdc)
{
	for (int k = 0; k < vHahuyeon.size(); k++)
	{
		if (isTurn)
		{
			if (isAtk)
			{
				vHahuyeon[k].atkImg->aniRender(hdc, vHahuyeon[k].rc.left - 8, vHahuyeon[k].rc.top - 8, playerAni);
			}
			else if (isHit)
			{
				vHahuyeon[k].blockImg->frameRender(hdc, vHahuyeon[k].rc.left, vHahuyeon[k].rc.top, 0, 4);

				HFONT myFont = CreateFont(13, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET, 0, 0, 0, 0, "나눔고딕체");
				HFONT oldFont = (HFONT)SelectObject(hdc, myFont);
				SetTextColor(hdc, RGB(255, 255, 255));
				sprintf_s(str, "%d", COLLISIONMANAGER->getDamage());
				TextOut(hdc, vHahuyeon[k].rc.left, vHahuyeon[k].rc.top, str, strlen(str));
		...
		if (isClick)
		{
			if (KEYMANAGER->isStayKeyDown(VK_LBUTTON))
			{
				...
				if (PtInRect(&rcMenu[4], m_ptMouse))	//취소
				{
					...
					vHahuyeon[k].rc.left = prevTile.rc.left;
					vHahuyeon[k].rc.right = prevTile.rc.right;
					vHahuyeon[k].rc.top = prevTile.rc.top;
					vHahuyeon[k].rc.bottom = prevTile.rc.bottom;
...
void Hahuyeon::playerCollision()
{
	for (int k = 0; k < vHahuyeon.size(); k++)
	{
		...
		if (IntersectRect(&temp, &vHahuyeon[k].rcAtk[0], &ENEMYMANAGER->getYeopo()->getEnemyVector()[0].rc) ||
			IntersectRect(&temp, &vHahuyeon[k].rcAtk[1], &ENEMYMANAGER->getYeopo()->getEnemyVector()[0].rc) ||
			IntersectRect(&temp, &vHahuyeon[k].rcAtk[2], &ENEMYMANAGER->getYeopo()->getEnemyVector()[0].rc) ||
			IntersectRect(&temp, &vHahuyeon[k].rcAtk[3], &ENEMYMANAGER->getYeopo()->getEnemyVector()[0].rc))
		{
			isTarget = true;
			frameX = 1;
