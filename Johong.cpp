...
void Johong::update()
{
	if (isTurn)
	{
		if (PLAYERMANAGER->getPturn())
		{
			...
			{
				if (playerVector[i]->getPlayerInfo().name == "조홍")
				{
					continue;
				}
...
void Johong::playerAnimation()
{
	if (isTurn)
	{
		...
		else
		{
			...
			{
				ANIMATIONMANAGER->addAnimation("playerHp", "조홍", 12, 13, 2, false, true);
...
{
	//플레이어 위치
	positionX = johong.rc.left / TILE_WIDTH;
	positionY = johong.rc.top / TILE_HEIGHT;
