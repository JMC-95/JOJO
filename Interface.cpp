...
void Interface::friendInformation(HDC hdc)
{
	for (int i = 0; i < FRIENDMANAGER->getFriend().size(); i++)
	{
		...
		if (friendly->getIsClick()) IMAGEMANAGER->frameRender("¸Þ´º", hdc, friendInfo.rc.left - 100, friendInfo.rc.top - 35, friendly->getFrameX(), 0);
...
void Interface::enemyInformation(HDC hdc)
{
	for (int i = 0; i < ENEMYMANAGER->getEnemy().size(); i++)
	{
		...
		if (enemy->getIsClick()) IMAGEMANAGER->frameRender("¸Þ´º", hdc, enemyInfo.rc.left - 100, enemyInfo.rc.top - 35, enemy->getFrameX(), 0);
		...
		if (PtInRect(&enemyInfo.rc, m_ptMouse))
		{
			...
			if (enemyInfo.name == "¿©Æ÷") enemyInfo.asstImg->render(hdc, 1134, 453);
